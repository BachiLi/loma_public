...
void mpi_runner(int num_children, _dfloat* input_array, int input_size, _dfloat* output) {
    MPI_Init(NULL, NULL);
    ...
    if (world_rank == 0) {
        ...
        int count = input_size* sizeof(_dfloat);
        // Communicate with children and send input and receive responses
        for (int i = 0; i < num_children; i++) {
            // Send input to child process
            MPI_Send(&input_size, 1, MPI_INT,i, 0, child_comm);
            MPI_Send(input_array,count, MPI_BYTE, i, 0, child_comm);
        }
