...
Given a function \lstinline{f(x0 : In[T0], x1 : Out[T1], ...) -> T}, reverse mode will transform the signature to \lstinline{DTf(x0 : In[T0], _dx0 : Out[T0], _dx1 : In[T1], ..., _dout : In[T])}. For an input argument \lstinline{x}, there are two cases:
\begin{itemize}
	...
	\item \lstinline{_dx : Out[T]} is only used as an output. We transform the argument to an adjoint input \lstinline{_dx : In[T]]}. The original output argument is removed.
\end{itemize}
...
\begin{lstlisting}[language=Python]
def foo(x : In[float], _dx_Ibx3EY : Out[float], _dy_oytqns : In[float], _dreturn_W7TFYQ : In[float]):
    # ...
