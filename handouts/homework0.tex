...
A function can take inputs and outputs that are of \lstinline{Array} types. You can have a struct of arrays or array of structs (or structs of arrays of structs). An important assumption we make is that the arrays of function arguments do not have \href{https://en.wikipedia.org/wiki/Aliasing_(computing)}{aliasing} (Fortran compilers make the same assumption and this is why Fortran is sometimes faster than C). We have not implemented a compile time check for this, so the users would have to be extra careful when passing array arguments.

Only \lstinline{Array} types can be marked as \lstinline{Output} (you can make your variables a size 1 array). If an argument is marked as input, we prohibit writing into the variable (i.e., it cannot be on the left hand side of assignment). If an argument is marked as output, we prohibit reading from the variable. These are all restrictions to simplify the implementation of reverse-mode differentiation. \TODO{implement the check}
