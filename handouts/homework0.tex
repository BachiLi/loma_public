...
You can clone the repo at \url{https://github.com/BachiLi/loma_public}. We include \href{https://github.com/shader-slang/slangpy}{slangpy} as a submodule, so you want to clone the repo using 
\begin{lstlisting}[language=bash]
git clone --recurse-submodules https://github.com/BachiLi/loma_public
\end{lstlisting}
...
\begin{itemize}
    ...
    \item \textbf{Slang}: we use \href{https://shader-slang.org/}{Slang} and its Python binding \href{https://slangpy.shader-slang.org/}{SlangPy} for generating parallel GPU code. See below for the installation instruction.
\end{itemize}

For the slang backend, we ask you to build and install slangpy using our submodule
\begin{lstlisting}[language=bash]
cd slangpy
pip install .
\end{lstlisting}

Once you have installed everything, run our tests to see if everything works.
\begin{lstlisting}[language=bash]
cd tests
python test.py
\end{lstlisting}
...
When you feed this program to the loma compiler and specifies C as a backend (currently, the two other available backends are ISPC and Slang, we will get to them later), the compiler will emit the following C code:
...
The target of the assign statement can be a member of a struct or an element of an array. For example,
\begin{lstlisting}
Assign(target=ArrayAccess(array=Var(id='a'), index=ConstInt(val=1)),
       val=ConstInt(val=6))
Assign(target=StructAccess(struct=Var(id='b'), member+id='c'),
       val=ConstInt(val=7))
\end{lstlisting}
means
\begin{lstlisting}
...
The type of the target should match the right hand side. If the types of left hand side and right hand side are \lstinline{int} and \lstinline{float} respectively, our type checker automatically inserts a type casting operation \lstinline{float2int} or \lstinline{int2float} for the conversion.
...
Given a loma program, we will ``lower'' it to a target language in the backends. Currently loma supports three backends: C, \href{https://ispc.github.io/index.html}{ispc}, and \href{https://shader-slang.org/}{Slang}. Let's talk about the C backend here, and we will discuss ispc and Slang backends after we discuss the parallelization model of loma.
...
\section{SIMD Parallelism and the Slang backend}

As mentioned, to use loma in practice, it's more or less necessary these days to support some form of parallelism. We adopt a SIMD parallelism model similar to how CUDA, OpenCL, and Compute Shaders work. For example, the following loma program parallely adds two integer arrays \lstinline{x} and \lstinline{y} and store the results into the array \lstinline{z}.
\begin{lstlisting}[language=python] 
@simd
...
If compiling using the C backend, loma generates the equivalent C code to above and simulate the parallelism serially. More interesting things happen if we specify other backends. Specifically, loma supports two other backends: \href{https://ispc.github.io/}{ISPC} and \href{https://shader-slang.org/}{Slang}. We'll focus the discussion on the Slang backend, but the ISPC backend works similarly (read the ISPC documentation in their webpage if you're interested in it).

The loma code above would be translated into the following Slang code.
\begin{lstlisting}[language=c]
[shader("compute")]
[numthreads(64, 1, 1)]
void parallel_add(uint _tid: SV_DispatchThreadID,
                  uniform int _total_threads,
                  StructuredBuffer<int> x,
                  StructuredBuffer<int> y,
                  RWStructuredBuffer<int> z) {
    if (_tid >= _total_threads) return;
    int i = _tid;
    z[i] = x[i] + y[i];
}
\end{lstlisting}

The syntax of Slang is similar to \href{https://learn.microsoft.com/en-us/windows/win32/direct3dhlsl/dx-graphics-hlsl}{HLSL} and the code above is basically an HLSL compute shader.

In Slang, \lstinline{[shader("compute")]} marks a function as a compute shader (similar to the \lstinline{@simd} decorator in loma). This allows a host program outside of Slang to call the kernel. Functions not marked as kernel can only be called by functions inside the same Slang program. The \lstinline{numthreads} decorator specifies how many GPU threads are counted as one group (typically 32 or 64 are good numbers). The \lstinline{_tid} argument gets the ID of the current thread (similar to \lstinline{thread_id()} in loma). 

As you can see, the generation of the Slang compute shader is basically the same as a C code, so we can inherit most of our C generation code. See \lstinline{codegen_slang.py} for reference.

Calling the Slang compute shader from our Python host code is more complicated though. Following is the code:
\begin{lstlisting}[language=python]
...
\end{lstlisting}
We use Slang's Python binding \href{https://slangpy.shader-slang.org/}{slangpy} for wrapping Slang APIs in Python. We further define \lstinline{slang_utils.create_slang_device()} for using the flags we want. Let's look at the code line by line:
\begin{lstlisting}[language=python]
slang_device = slang_utils.create_slang_device()
\end{lstlisting}
This line creates a Slang \href{https://slangpy.shader-slang.org/en/latest/src/api_reference.html#slangpy.Device}{Device} and selects the backend (Metal for MacOS and Vulkan for other platforms). The code is simply
\begin{lstlisting}[language=python]
...
with open('loma_code/parallel_add.py') as f:
    module, kernels = compiler.compile(f.read(),
                                       ...
                                       slang_device = slang_device)
\end{lstlisting}
are similar to the C backend, except we pass the slang Device to the compile function. The compile function returns a \href{https://slangpy.shader-slang.org/en/latest/src/api_reference.html#slangpy.Module}{slangpy.Module} and a \href{https://slangpy.shader-slang.org/en/latest/src/api_reference.html#slangpy.ComputeKernel}{slangpy.ComputeKernel}.

Next, we create the Slang buffers for the inputs and outputs:
\begin{lstlisting}[language=python]
buffer_x = slangpy.Tensor.from_numpy(
    device=slang_device,
    ndarray=np.array([2, 3, 5], dtype=np.int32)
...
\end{lstlisting}
See the slangpy \href{https://slangpy.shader-slang.org/en/latest/src/tensors/python.html}{tutorial} for more information about \lstinline{slangpy.Tensor}.

Next, we launch the Slang compute kernel. \lstinline{kernels} is a Python dictionary that contains all the compute kernels in the file.
\begin{lstlisting}[language=python]
kernels['parallel_ad'].dispatch(thread_count=[3, 1, 1],
                _total_threads=3,
                x=buffer_x.storage,
                y=buffer_y.storage,
                z=buffer_z.storage)
...
Lastly, we read from the buffer (the copying of the results from GPU to CPU is handled by slangpy) and compare the expected results.
\begin{lstlisting}[language=python]
z = buffer_z.to_numpy().view(np.int32)
assert z[0] == 9 and z[1] == 14 and z[2] == 18
\end{lstlisting}
...
def my_atomic_add(x : In[Array[float]],
    ...
    atomic_add(z, x[i])
\end{lstlisting}
This loma code will sum up all elements of \lstinline{x} up until the number of threads launched. Internally, for C backend, this just turns into \lstinline{z = z + x[i]}. 
If the first argument is an output variable,
for the ISPC backend, we use an atomic \href{https://en.wikipedia.org/wiki/Compare-and-swap}{Compare and Swap} loop to implement atomic add (note that this is known to be very slow, however faster instructions require specific hardware like an NVIDIA GPU).
For the Slang backend, we use \href{https://docs.shader-slang.org/en/stable/external/core-module-reference/global-decls/interlockedadd-0b.html}{InterlockedAdd}.
If the first argument is not an output variable (i.e., it's a local variable declared per thread), then it still turns into \lstinline{z = z + x[i]} even for ISPC/Slang backends.
...
\lstinline{examples/raytrace_slang.py} and \lstinline{examples/loma_code/raytrace_simd.py} further shows the same program but targeted for the Slang backend. It demonstrates how Slang interacts with the custom structs we defined in loma.
