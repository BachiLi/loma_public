...
Loma also supports call-by-reference (similar to \lstinline{int &w} in C++) using the \lstinline{is_byref} flag. If an argument is marked as \lstinline{is_byref}, then even if it is not an array, when a calle modifies the variable, the calle will observe the changes.
