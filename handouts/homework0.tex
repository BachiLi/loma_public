...
An example loma program looks like this (the file is located in \lstinline{xamples/loma_code/sum_array.py}):
...
A function can take inputs and outputs that are of array types. You can have a struct of arrays or array of structs (or structs of arrays of structs). An important assumption we make is that the arrays of function arguments do not have \href{https://en.wikipedia.org/wiki/Aliasing_(computing)}{aliasing} (Fortran compilers make the same assumption and this is why Fortran is sometimes faster than C). We have not implemented a compile time check for this, so the users would have to be extra careful when passing array arguments.
...
\todo{update this code snippet after we implement more checking.}
\begin{lstlisting}
