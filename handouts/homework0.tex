...
slang_device = slang_utils.create_slang_device()
\end{lstlisting}
This line creates a Slang \href{https://slangpy.shader-slang.org/en/stable/src/api_reference.html#slangpy.Device}{Device} and selects the backend (Metal for MacOS and Vulkan for other platforms). The code is simply
\begin{lstlisting}[language=python]
...
with open('loma_code/parallel_add.py') as f:
    structs, kernel = compiler.compile(f.read(),
                                       ...
                                       slang_device = slang_device)
\end{lstlisting}
are similar to the C backend, except we pass the slang Device to the compile function.
...
Next, we launch the Slang kernel.
\begin{lstlisting}[language=python]
kernel.dispatch(thread_count=[3, 1, 1],
                _total_threads=3,
                x=buffer_x,
